...
static int get_server_address_and_port(char **p_ws_addr, int *p_ws_port, const char *base_path)
{
    ...
    p = strstr(addr_and_port_string, WS_BASE_PATH_DELIM);
    if (NULL == p) {
    ...
    int port_string_length = strlen(addr_and_port_string) - strlen(WS_BASE_PATH_DELIM) - strlen(port_string);
...
wsclient_t *wsclient_create(const char *base_path, sslConfig_t * ssl_config, int ws_log_mask)
{
    if (!base_path) {
